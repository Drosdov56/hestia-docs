# Index — documentation de référence Hestia

Porte d’entrée de **hestia-docs**. Aucun contenu dupliqué : renvois uniquement.

## Fondements

| Besoin | Document |
|--------|----------|
| **Pourquoi Hestia est conçu ainsi** | [Constitution — Document de réflexion architecturale](constitution/HESTIA%20-%20DOCUMENT%20DE%20RÉFLEXION%20ARCHITECTURALE.md) |
| Définitions officielles | [GLOSSAIRE.md](gouvernance/GLOSSAIRE.md) |
| Gouvernance documentaire | [DECISION-0001-DOCUMENTATION.md](gouvernance/DECISION-0001-DOCUMENTATION.md) |
| Classification migration | [CLASSIFICATION-DEFINITIVE.md](gouvernance/CLASSIFICATION-DEFINITIVE.md) |

## Écosystème & architecture

| Besoin | Document |
|--------|----------|
| Organisation multi-dépôts | [ecosysteme.md](ecosysteme/ecosysteme.md) |
| Flux domotique / passerelle | [architecture-domotique.md](architecture/architecture-domotique.md) |
| Carte des modèles conceptuels | [ARCHITECTURE-CONCEPTUELLE.md](modeles/ARCHITECTURE-CONCEPTUELLE.md) |
| Vision fonctionnelle | [FUNCTIONAL-VISION.md](vision/FUNCTIONAL-VISION.md) |
| **Reconnexion autonome nœud ↔ serveur (AUTO-001)** | [Spécification](architecture/AUTO-001-reconnection-autonome-noeud.md) · [Revue d’architecture](architecture/AUTO-001-revue-architecture.md) · [Suivi d’exécution](backlog/execution/AUTO-001.md) |
| **Supervision / admin nœuds (AUTO-002)** | [Spécification](architecture/AUTO-002-supervision-administration-noeuds.md) · [Identité 002F](architecture/AUTO-002F-identite-cycle-vie-noeud.md) · [Suivi](backlog/execution/AUTO-002.md) |
| Vocabulaire nœud (`node_id`) | [GLOSSAIRE.md](gouvernance/GLOSSAIRE.md) § Identité |

## Modèles conceptuels

| Modèle | Document |
|--------|----------|
| Information | [MODELE-INFORMATION.md](modeles/MODELE-INFORMATION.md) |
| Décision | [MODELE-DECISION.md](modeles/MODELE-DECISION.md) |
| Identité (personnes) | [MODELE-IDENTITE.md](modeles/MODELE-IDENTITE.md) |
| Habitat | [MODELE-HABITAT.md](modeles/MODELE-HABITAT.md) |
| Foyer | [MODELE-FOYER.md](modeles/MODELE-FOYER.md) |
| Capteur (contrat doc) | [MODELE-CAPTEUR.md](modeles/capteurs/MODELE-CAPTEUR.md) |
| Exemple matériel SNZB-06P24 | [SNZB-06P24.md](modeles/capteurs/SNZB-06P24.md) |

## Équipements

| Besoin | Document |
|--------|----------|
| Cycle de vie / SoT équipement | [70-cycle-vie-equipements.md](equipements/70-cycle-vie-equipements.md) |
| ADR mise en service | [ADR-004](adr/ADR-004-mise-en-service-equipements.md) |
| ADR cycle de vie | [ADR-005](adr/ADR-005-cycle-vie-equipements.md) |

## ADR transverses

| ADR | Sujet |
|-----|-------|
| [ADR-020](adr/ADR-020%20-%20Positionnement%20de%20Hestia%20vis-%C3%A0-vis%20de%20Home%20Assistant.md) | Positionnement vs Home Assistant |
| [ADR-018](adr/ADR-018-architecture-domotique-agent-passerelle.md) | Architecture agent / passerelle |
| [ADR-021](adr/ADR-021-identite-credentials-noeud.md) | Identité et credentials nœud (token par nœud) |
| [ADR-023](adr/ADR-023-terminal-distant-websocket-sortant.md) | Terminal distant (WS sortant, relay dédié) |
| [ADR-004](adr/ADR-004-mise-en-service-equipements.md) | Mise en service équipements |
| [ADR-005](adr/ADR-005-cycle-vie-equipements.md) | Cycle de vie équipements |

## Archive & audits

| Type | Lien |
|------|------|
| Historique non normatif | [archive/](archive/) |
| Audits pré-migration | [audits/](audits/) |

## Pilotage développement

| Besoin | Document |
|--------|----------|
| **Backlog officiel** | [backlog/README.md](backlog/README.md) |
| Inventaire Epics / Features | [backlog/BACKLOG.md](backlog/BACKLOG.md) |
| Ordre d’implémentation | [backlog/ROADMAP.md](backlog/ROADMAP.md) |
| Dépendances | [backlog/DEPENDANCES.md](backlog/DEPENDANCES.md) |
| Clôture EPIC-001 | [backlog/execution/REPORT-CLOTURE-EPIC-001.md](backlog/execution/REPORT-CLOTURE-EPIC-001.md) |

## Documentation locale (hors ce dépôt)

| Dépôt | Entrée |
|-------|--------|
| hestia | `docs/` du dépôt applicatif |
| hestia-installer | `docs/INDEX.md` — installation, exploitation nœud, backlog technique local |
| hestia-agent | `docs/ARCHITECTURE.md` |

### OPS nœud (hors hestia-docs)

| Besoin | Document |
|--------|----------|
| **OPS-001** — exploitation / `hestia-ops`, résilience, cold boot | Dépôt **hestia-installer** → `docs/INSTALL.md` (procédure locale) |
| Distinction admin fonctionnelle vs OPS | [GLOSSAIRE.md](gouvernance/GLOSSAIRE.md) § Administration |

> Source de vérité produit : `vision`, `backlog`, `roadmap` et `validation terrain` sont portés par ce dépôt.  
> Les dépôts de code conservent uniquement leur documentation locale d’implémentation, d’exploitation ou de runtime.
